...
class CanvasPainter {
  ...
  // Verschuift de oorsprong — gebruikt voor de witte print-veiligheidsrand
  // (zie MapRender.render): alle daaropvolgende tekenopdrachten vallen zo
  // automatisch binnen het ingesprongen vlak, zonder dat elke coördinaat in
  // de rest van het tekenpad zelf hoeft te weten dat die rand bestaat.
  translate(dx, dy) { this.ctx.save(); this.ctx.translate(dx, dy); }
  restoreTranslate() { this.ctx.restore(); }
}
...
class SVGPainter {
  ...
  translate(dx, dy) { this.parts.push(`<g transform="translate(${fmt(dx)},${fmt(dy)})">`); }
  restoreTranslate() { this.parts.push('</g>'); }
